...
def generate_markdown_table(results_dict):
    ...
    faiss_row = f"| {'**FAISS**':<14} | {faiss_build:<14} | {faiss_search:<15} | {faiss_recall:<14} | {'—':<21} | {'—':<21} | {'😎 *Just works.*':<31} |"
    bf_row =    f"| {'**m2vdb (BF)**':<14} | {bf_build:<14} | {bf_search:<15} | {bf_recall:<14} | {bf_throughput:<21} | {bf_vs_faiss:<21} | {'😬 *Please do not look.*':<31} |"
    ann_row =   f"| {'**m2vdb (ANN)**':<14} | {ann_build:<14} | {ann_search:<15} | {ann_recall:<14} | {ann_throughput:<21} | {ann_vs_faiss:<21} | {'😐 *Kind of works?*':<31} |"
...
def run_benchmark():
    ...
    m2v_bf_throughput = len(xq) / m2v_bf_search_time if m2v_bf_search_time > 0 else 0
    ...
    m2v_ann_throughput = len(xq) / m2v_ann_search_time if m2v_ann_search_time > 0 else 0
    ...
    print("\nResults (Console):")
    print(f"{'Method':<15} {'Build':<12} {'Search':<12} {'Recall@10':<10} {'Throughput':<10} {'RAM(MB)':<10} {'Disk':<10} {'vs FAISS'}")
    print(f"{'FAISS':<15} {format_time(faiss_build_time):<12} {format_time(faiss_search_time):<12} {faiss_recall:.4f}    {'—':<10} {'—':<10} {'—':<10} {'—'}")
    print(f"{'m2vdb (BF)':<15} {format_time(m2v_bf_build_time):<12} {format_time(m2v_bf_search_time):<12} {m2v_bf_recall:.4f}    {m2v_bf_throughput:<10.1f} {mem_usage:<10.1f} {disk_footprint:<10} {bf_search_slowdown:+.1f}%")
    print(f"{'m2vdb (ANN)':<15} {format_time(m2v_ann_build_time):<12} {format_time(m2v_ann_search_time):<12} {m2v_ann_recall:.4f}    {m2v_ann_throughput:<10.1f} {mem_usage:<10.1f} {disk_footprint:<10} {ann_search_slowdown:+.1f}%")
    ...
    # Generate and Update Markdown Table
